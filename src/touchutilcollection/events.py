...
_subscriptions:Dict[str, Dict[str, List[EventCallbackType]]] = {} 


def subscribe( emitter:OP, event:str, callback:EventCallbackType):
    _subscriptions.setdefault( emitter.path , {}).setdefault(event,  []).append( callback )

def unsubscribe( emitter:OP, event:str, callback:EventCallbackType):
    _subscriptions.setdefault( emitter.path , {}).setdefault(event,  []).remove( callback )
