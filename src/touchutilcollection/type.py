...
def op_as(pattern:Union[str, int, Tuple[Union[str, int], ...]], asType:Type[T], includeUtility = False): # pyright: ignore[reportInvalidTypeVarUse] 
...
def op_as_ex(pattern:Union[str, int, Tuple[Union[str, int], ...]], asType:Type[T], includeUtility = False): # pyright: ignore[reportInvalidTypeVarUse] 
	# T is in use. This is only a utility function!
	if is_tuple( pattern ): _pattern = pattern
	else: _pattern = (pattern,)
